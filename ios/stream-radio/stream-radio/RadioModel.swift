...
final class StationStore {
    ...
    func toggleFavorite(_ station: Station) {
        // 用 url 定位：传入的可能是播放器持有的旧值副本（isFavorite 已过期），
        // 整值相等（含 isFavorite）匹配会失败，故按稳定的业务键 url 匹配
        guard let i = stations.firstIndex(where: { $0.url == station.url }) else { return }
    ...
    static func parseM3U(_ text: String) -> [Station] {
        ...
        for raw in cleaned.components(separatedBy: .newlines) {
            ...
            // #EXTINF 处理；其余 # 注释行（#EXTM3U / #update: / #EXTGRP: 等）统一在下方跳过
